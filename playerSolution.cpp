...
class Player{
    ...
    // removing LAST player from Real Madrid
      for(int i = vec.size() - 1; i >= 0; i--){
                Player currentPlayer = vec[i];

                if(currentPlayer.getTeam() == REAL_MADRID){
                    vec.erase(vec.begin() + i);
                    return; //break
                }

            }
    // removing second to last player from Real Madrid (penúltimo):


    //    bool foundFirst = false;
    //         for(int i = vec.size() - 1; i >= 0; i--){
    //             Player currentPlayer = vec[i];
    //             if(foundFirst && currentPlayer.getTeam() == REAL_MADRID){
    //                 vec.erase(vec.begin() + i);
    //                 return; //break
    //             }
    ...
    */
       static vector<Player> combine(vector<Player> v1, vector<Player> v2){
        ...
        for(int i = 0; i < v1.size(); i++){
            Player playerVector1 = v1[i];
            int secondVectorIndex = v1.size() - 1 - i;
            Player playerVector2 = v2[secondVectorIndex];

            result.push_back(playerVector1);
            result.push_back(playerVector2);
        }
       return result;
       }
...
int main(){
   ...
   showVector(playerVector);
    ...
    cout << endl << endl<< "Exercise 2 After combining: " << endl;
